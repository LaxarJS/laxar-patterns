...
import { createAxEventBusMock, createAxLogMock } from 'laxar/laxar-widget-service-mocks';
...
describe( 'A didReplace-event publisher with missing feature config', () => {
   ...
   describe( 'by default', () => {
      ...
      beforeEach( () => {
         try {
            publisher = resources.replacePublisherForFeature( context, 'veryImportantFeature' );
         }
         catch( e ) {
            exception = e;
         }
      ...
      it( 'fails on creation', () => {
         expect( publisher ).not.toBeDefined();
         expect( exception.message ).toEqual(
            'Assertion error: Code should be unreachable! Details: replacePublisherForFeature: ' +
            'missing configuration for "features.veryImportantFeature.resource"'
         );
      } );
   ...
   describe( 'and options.isOptional', () => {
...
describe( 'A did replace-publisher with replaceRequest handling', () => {
   ...
   describe( 'configured with default behavior', () => {

      beforeEach( () => {
         publisher = resources.replacePublisherForFeature( context, 'lunch' );

         publisher( { type: 'camembert' } );
      } );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      describe( 'on replaceRequest for the specific resource', () => {

         beforeEach( done => {
            context.eventBus.publish.calls.reset();
            context.eventBus.publish( 'replaceRequest.cheese', { resource: 'cheese' }, { sender: 'test' } );
            context.eventBus.drainAsync().then( done );
         } );

         it( 'publishes the resource again', () => {
            expect( context.eventBus.publish ).toHaveBeenCalledWith( 'didReplace.cheese', {
               resource: 'cheese',
               data: { type: 'camembert' }
            }, { deliverToSender: false } );
         } );
      ...
      describe( 'on an unspecific replaceRequest', () => {

         beforeEach( done => {
            context.eventBus.publish.calls.reset();
            context.eventBus.publish( 'replaceRequest', {}, { sender: 'test' } );
            context.eventBus.drainAsync().then( done );
         } );
   ...
   describe( 'configured as false', () => {

      beforeEach( done => {
         publisher = resources.replacePublisherForFeature( context, 'lunch', {
            replaceRequestHandler: false
         } );
      ...
      it( 'ignores specific or unspecific replaceRequest event', () => {
         expect( context.eventBus.publish )
            .not.toHaveBeenCalledWith( 'didReplace.cheese', jasmine.any( Object ), jasmine.any( Object ) );
      } );
   ...
   describe( 'configured with a custom handler', () => {

      let replaceRequestHandler;
      let resolvePromise;

      beforeEach( () => {
         replaceRequestHandler = jasmine.createSpy( 'replaceRequestHandler' )
            .and.returnValue( new Promise( resolve => {
               resolvePromise = resolve;
            } ) );
         publisher = resources.replacePublisherForFeature( context, 'lunch', { replaceRequestHandler } );
      ...
      describe( 'on replaceRequest for the specific resource', () => {
         ...
         beforeEach( done => {
            ...
            context.eventBus.publish( 'replaceRequest.cheese', { resource: 'cheese' }, { sender: 'test' } );
            context.eventBus.drainAsync().then( done );
            resolvePromise( { type: 'gouda' } );
         } );

         it( 'calls the handler with the event and meta object', () => {
            expect( replaceRequestHandler )
               .toHaveBeenCalledWith( { 'resource': 'cheese' }, jasmine.any( Object ) );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'sends a didReplace event for the returned resource data', () => {
            expect( context.eventBus.publish ).toHaveBeenCalledWith( 'didReplace.cheese', {
               resource: 'cheese',
               data: { type: 'gouda' }
            }, { deliverToSender: false } );
