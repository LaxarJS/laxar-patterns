...
function VisibilityHandler( scope, optionalOptions ) {
   ...
   if( options.onAnyAreaRequest ) {
      const requestEvent = `changeAreaVisibilityRequest.${scope.widget.id}`;
      scope.eventBus.subscribe( requestEvent, responder( this, options.onAnyAreaRequest ) );
   }

   const didEvent = `didChangeAreaVisibility.${scope.widget.area}`;
   scope.eventBus.subscribe( didEvent, event => {
      const wasVisible = scope.isVisible || false;
...
function requestPublisherForWidget( scope ) {
   return visible => {
      const eventName = `changeWidgetVisibilityRequest.${scope.widget.id}.${visible}`;
...
function requestPublisherForArea( scope, area ) {
   return visible => {
      ...
      return scope.eventBus.publishAndGatherReplies( eventName, { area, visible }, meta );
   };
}
