...
define( [
    ...
    *
    * @param {Object} scope the scope the handler works on
    * @param {String} modelKey the property of `scope.model` / `scope,resources` the handler writes replaces to
    ...
    *
    * @param {Object} scope the scope the handler works on
    * @param {String} modelKey the property of `scope.model` / `scope,resources` the handler applies updates to
    * @returns {Function} the handler function
    */
    ...
    * @param {String} featurePath the property of `scope.model` the publisher reads the resource name from
    * @returns {Function} the publisher function. Takes the data to publish as single argument.
    */
    ...
    * `ResourceHandler.prototype.registerResourceFromFeature`.
    *
    * The returned function can be used in conjunction with `$scope.$watch` or an ngChange-observer. It can
    * take one or two arguments. The first one is a map of updates as described in TS01 - Events chapter 2.4.2
    * "didUpdate.<resource>". The second one a complete data object representing the full resource with
    * updates already applied. Any combination of them being `null` or defined is possible, were the cases of
    * both being defined or `null` don't make much sense.
    *
    * @param {Object} scope the scope the publisher works on
    * @param {String} featurePath the property of `scope.model` the publisher reads the resource name from
    * @returns {Function} the publisher function
    */
   function updatePublisherForFeature( scope, featurePath ) {
      assert( scope ).hasType( Object ).isNotNull();
      assert( scope.eventBus ).hasType( Object ).isNotNull();
    ...
    * @returns {ResourceHandler} not `null`
    */
   ...
   ResourceHandler.prototype = {
       ...
       *
       * @param {String} featurePath the attribute path to the feature for the resource
       ...
       * @param {Boolean} options.omitFirstReplace if `true` `options.onReplace` is only called after the
       *     first time a didReplace event occurred. Default is `false`
       * @param {String} options.modelKey the key to use for the resource in `scope.model` / `scope.resources`.
       *     If not given the last path fragment of `featurePath` is used. For example if the path is
       *     'myfeature.superResource` the key will be "superResource".
       * @returns {ResourceHandler} this instance
       */
      registerResourceFromFeature: function( featurePath, options ) {
         var resource = ax.object.path( this.scope_.features, featurePath + '.resource' );
         assert( resource ).isNotNull( 'Could not find resource configuration in features for "' + featurePath + '"' );

         options = ax.object.deepClone( options || {} );
         if( !options.modelKey ) {
       ...
       * will be used.
       *
       * @param {String} resource the resource the handler should be registered for
       * @param {Object=} options options and callbacks to use
       * @param {Function|Function[]} options.onReplace
       *    a function or a list of functions to call when a didReplace event is received. Each function
       *    receives the event object as argument. If `options.omitFirstReplace` is `true`, it is only called
       ...
       *    first time a didReplace event occurred. Default is `false`
       * @param {String} options.modelKey the key to use for the resource in `scope.model` / `scope.resources`.
       *    If not given the value of `resource` is used.
       * @returns {ResourceHandler} this instance
       */
       ...
       * @returns {ResourceHandler} this instance
       */
      whenAllWereReplaced: function( callback ) {
         assert( callback ).hasType( Function ).isNotNull();

         this.allReplacedCallback_ = _.once( callback );
    ...
    * @param {Function|Function[]} funcOrArray
    */
    ...
    *
    * @param {Object} resourceA the first object to test
    * @param {Object} resourceB the second object to test
    * @param {String[]} attributes the list of attributes determining resource identity
    * @return {Boolean} `true` if both objects are assumed to represent the same resource, `false` otherwise
    */
