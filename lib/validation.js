 ...
 *   It will get a default level of *ERROR*.
...
export function successEvent( resource, ...messages ) {
   return createEvent( resource, messagesFromArgs( messages ), 'SUCCESS' );
}
...
export function errorEvent( resource, ...messages ) {
   return createEvent( resource, messagesFromArgs( messages ), 'ERROR' );
}
 ...
 *    it can do the event handling, and a `log` property for logging
