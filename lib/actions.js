 ...
 */
export function publisherForFeature( scope, feature, optionalOptions ) {
   const action = object.path( scope.features, `${feature}.action`, null );
 ...
 * Interpretation is simple: If at least one `didTakeAction` event yields the outcome "ERROR",
 ...
 *    the publisher as described above
 */
export function publisher( scope, action, optionalOptions ) {
   ...
   return optionalEvent => {
      ...
      return scope.eventBus
         ...
         .then( didResponses => {
            const failed = didResponses.some( response => {
 ...
 */
export function connectPublisherToFeature( scope, feature, optionalOptions ) {
   const topic = object.path( scope.features, feature );
   const publisher = topic ? publisherForFeature( scope, feature, optionalOptions ) : () => {};
   object.setPath( scope, `actions.${feature}`, publisher );
}
 ...
 */
export function handlerFor( scope ) {
   assert( scope ).hasType( Object ).hasProperty( 'eventBus' );
...
ActionHandler.prototype.registerActionsFromFeature = function( feature, handler ) {
   const actions = object.path( this.scope.features, `${feature}.onActions` ) || [];
...
ActionHandler.prototype.registerActions = function( actions, handler ) {
   ...
   actions.forEach( action => {
      self.scope.eventBus.subscribe( `takeActionRequest.${action}`, ( event, meta ) => {
