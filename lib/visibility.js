...
define( [
   ...
   function VisibilityHandler( scope, optionalOptions ) {
      ...
      if( options.onAnyAreaRequest ) {
         ...
         scope.eventBus.subscribe( requestEvent, function( event ) {
            var result = options.onAnyAreaRequest( event );
            if( result === true || result === false ) {
               var didEvent = [ 'didChangeAreaVisibility', event.area, result ].join( '.' );
               scope.eventBus.publish( didEvent, { area: event.area, visible: result }, { deliverToSender: false } );
            }
         } );
      }
   ...
   VisibilityHandler.prototype = {
      ...
      registerArea: function( areaName, optionalOptions ) {
         ...
         if( options.onRequest ) {
            ...
            this.scope_ .eventBus.subscribe( requestEvent, options.onRequest );
         }
         return this;
      }
   };
