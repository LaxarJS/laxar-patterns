 ...
 *    a `laxar.string.format`-compatible function to interpolate named message arguments.
 *    It will always be called with `( message, [], data )` where `data` was passed to the
 *    publisher by the user
 *
 * @return {Function}
...
export function errorPublisherForFeature( context, featurePath, options ) {
   ...
   const formatter = ( options && options.formatter ) || string.format;
   assert( formatter ).hasType( Function );
   ...
   return ( code, messagePath, data = {}, cause = {} ) => {
      ...
      const message = formatter( localizer ? localizer( rawMessage ) : rawMessage, [], data );
