...
define( [ 'laxar' ], function( ax ) {
   ...
   function errorPublisherForFeature( scope, featurePath, options ) {
      ...
      return function( code, messagePath, data, cause ) {
         ...
         scope.eventBus.publish( 'didEncounterError.' + code, {
            code: code,
            message: format( localizer ? localizer( rawMessage ) : rawMessage, data ),
            data: data,
            cause: cause || {}
   ...
   function format( message, data ) {
      var args = [ message ].concat( Object.keys( data ).map( function( key ) {
         return text.argument( key, data[ key ] );
      } ) );
      return text.format.apply( text.format, args );
   }
