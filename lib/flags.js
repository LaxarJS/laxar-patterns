...
FlagHandler.prototype.registerFlagFromFeature = function( featurePath, optionalOptions ) {
   return this.registerFlag( object.path( this.scope_.features, featurePath, [] ), optionalOptions );
};
...
FlagHandler.prototype.registerFlag = function( possibleFlags, optionalOptions ) {
   ...
   optionalOptions = object.options( optionalOptions, {
      predicate: 'any'
   } );

   const scope = this.scope_;
   const applyToScope = 'scopeKey' in optionalOptions ?
      state => object.setPath( scope, optionalOptions.scopeKey, state ) : () => {};

   const flags = processFlags( possibleFlags );
   const changeHandler = processChangeHandlers( optionalOptions.onChange );
   let oldState = ( typeof optionalOptions.initialState === 'boolean' ) ?
   ...
   flags.forEach( function( flag ) {
      this.scope_.eventBus.subscribe( 'didChangeFlag.' + flag.name, function( event ) {
         ...
         const newState = evaluateState( flags, optionalOptions.predicate );
