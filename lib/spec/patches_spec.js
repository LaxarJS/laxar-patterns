...
define( [
   ...
   describe( 'patches.apply( object, patchMap )', function() {
      ...
      it( 'applies a map of patches for existing properties', function() {
         patches.apply( obj, {
            'listOfStrings.1': 'R',
            'level1.value': 'XXX',
      ...
      it( 'adds non existing paths for objects', function() {
         patches.apply( obj, {
      ...
      it( 'treats sparse arrays as patch map for all non undefined values', function() {
         var updates = [];
         updates[1] = 'X';
         updates[4] = 'Y';
      ...
      it( 'works when there are null values in the base object (jira ATP-7356)', function() {
         var obj = { myKey: null };
         var updates = { 'myKey.0': 12 };

         expect( function() { patches.apply( obj, updates ); } ).not.toThrow();
         expect( obj ).toEqual( { myKey: [ 12 ] } );
      } );

      /////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'ensures an order when applying patches (jira ATP-7483)', function() {
         var obj = {
            car: {
   ...
   describe( 'patches.create( target, subject )', function() {
      ...
      it( 'returns null if the first argument is no array or object', function() {
      ...
      it( 'returns a copy of the target if both arguments are of different type', function() {
      ...
      it( 'returns a patch map for new entries in a single level object', function() {
      ...
      it( 'returns a patch map for removed entries in a single level object', function() {
      ...
      it( 'returns a patch map for new entries in a multi level object', function() {
         var source = {
            listOfStrings: [ 'A', 'C' ],
            level1: {
               value: 'XXX',
      ...
      it( 'returns a patch map for deleted entries in a multi level object', function() {
         var target = {
            listOfStrings: [ 'A', 'C' ],
            level1: {
               value: 'XXX',
      ...
      it( 'can be used in conjunction with applyPatches', function() {
         var source = {
            listOfStrings: [ 'A', 'C' ],
            level1: {
               value: 'XXX',
      ...
      it( 'works when there are null values in the result object (jira ATP-7356)', function() {
         var base = { myKey: [ 12 ] };
         var result = { myKey: null };
         var updates;

         expect( function() { updates = patches.create( result, base ); } ).not.toThrow();
      ...
      it( 'ignores hidden properties in the object (LaxarJS/laxar_pattens#3)', function() {
         var base = { };
         var result = polluted;
         var updates;
   ...
   describe( 'patches.merge( first, second )', function() {
      ...
      it( 'removes changes in first overwritten by changes in second', function() {
         var one = {
            'x.a': 12,
            'y.1': 34
         };
      ...
      it( 'merges smaller changes of second into more general change of first', function() {
         var one = {
            'x.a': 666,
            y: [ 4, 5, 6 ]
         };
      ...
      it( 'merges simple object structures were one key is a prefix of another key (jira ATP-7693)', function() {
         var first = {
            payment: {
               amount: 10,
               currency: 'EUR'
