...
define( [
    ...
    * option), it additionally receives a *done* callback as third argument, which **must** be called when the
    * asynchronous task is finished. To deliver its outcome a synchronous handler can simply return the string
    * `'SUCCESS'` or `'ERROR'`, while the asynchronous handler needs to pass the string to the *done* callback
    * as first argument. in any case if the handler throws an exception (of course not within asynchronous
    * code) this is treated as outcome `'ERROR'` and the thrown exception will be rethrown to be handled by
    * the default event bus error handler. Any outcome that is not `'SUCCESS'` or `'ERROR'` is treated as
    * `'SUCCESS'`.
    *
   ...
   function callSynchronousHandler( eventBus, action, handler, event, meta ) {
      ...
      var outcome = ( returnValue === OUTCOME_ERROR || exception ) ? OUTCOME_ERROR : OUTCOME_SUCCESS;
      eventBus.publish( 'didTakeAction.' + action + '.'  + outcome, {
         action: action,
         outcome: outcome
      }, DELIVER_TO_SENDER );
   ...
   function callAsynchronousHandler( eventBus, action, handler, event, meta ) {
      ...
      try {
         handler( event, meta, function done( doneOutcome ) {
            var outcome = doneOutcome === OUTCOME_ERROR ? OUTCOME_ERROR : OUTCOME_SUCCESS;
            eventBus.publish( 'didTakeAction.' + action + '.'  + outcome, {
               action: action,
               outcome: outcome
            }, DELIVER_TO_SENDER );
